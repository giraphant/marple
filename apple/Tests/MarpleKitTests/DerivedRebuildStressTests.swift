...
@Suite struct DerivedRebuildStressTests {
    ...
    @Test func overlappingScheduledBuildsPublishLatestSnapshot() async throws {
        ...
        for _ in 0..<8 {
            _ = catalog.publish(snapshot, pass: catalog.beginStandalonePass())
            catalog.scheduleDeferredDerivedRebuild()
        ...
        _ = catalog.publish(Array(snapshot.prefix(3)), pass: catalog.beginStandalonePass())
        catalog.scheduleDeferredDerivedRebuild()
        if let task = catalog.deferredDerivedTask { tasks.append(task) }
