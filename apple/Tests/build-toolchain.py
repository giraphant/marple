...
with tempfile.TemporaryDirectory(prefix="marple-toolchain-test-") as temporary:
    ...
    sdk_root = developer / "MacOSX.sdk"
    sdk_root.mkdir()
    (sdk_root / "SDKSettings.plist").write_bytes(plistlib.dumps({"Version": "26.5"}))
    for name, source in {
        "xcrun": '''#!/bin/sh
...
if [ "$1" = "--show-sdk-path" ]; then printf "%s\\n" "$TEST_SDKROOT"; exit 0; fi
if [ "$1" = "vtool" ]; then printf "sdk %s\\n" "$TEST_BUILT_SDK"; exit 0; fi
printf "%s\\n" "$TEST_SDK"
''',
        "swift": '#!/bin/sh\nprintf "%s\\n" "$DEVELOPER_DIR" "$SDKROOT" "$@" > "$TEST_MARKER"\nexit "${TEST_SWIFT_EXIT:-42}"\n',
    }.items():
        path = binaries / name
    ...
    environment = dict(os.environ, CONFIG="release", VERSION="test", BUILD="test",
                       ...
                       TEST_SDKROOT=str(sdk_root), TEST_SWIFT_EXIT="42")
    for sdk in ("15.5", "26.5"):
        ...
        if sdk == "15.5":
            ...
            invocation = marker.read_text().splitlines()
            assert invocation[:2] == [str(developer), str(sdk_root)], invocation
            assert invocation[invocation.index("--sdk") + 1] == str(sdk_root), invocation
            assert invocation[-1] == sdk, invocation
    result = subprocess.run([str(script)], env=dict(environment, TEST_SDK="26.5",
                                TEST_SWIFT_EXIT="0", TEST_BUILT_SDK="15.0"), capture_output=True, text=True)
    assert result.returncode != 0 and "built SDK (15.0)" in result.stderr, result.stdout + result.stderr
    assert not Path(environment["APP_DIR"]).exists()
    marker.unlink(missing_ok=True)
    ...
    assert result.returncode != 0 and not marker.exists(), result.stdout + result.stderr
print("Release uses the selected Xcode/SDK and rejects old, missing, or mismatched SDKs.")
