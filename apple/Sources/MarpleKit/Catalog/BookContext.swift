...
/// book or one of its chapters, surface the book's overview plus the ordered
/// chapter list so the reader can jump between them. Mirrors the web
...
/// slug comes from the path for a book and from the `book` field for a
/// chapter; the overview is matched by that same path-derived slug and chapters
/// are the chapter entries carrying it, ordered by path.
public func bookContext(for entry: Entry, in entries: [Entry]) -> BookContext? {
    let slug: String?
    switch entry.type {
    case .book:    slug = bookSlug(entry.path)
    case .chapter: slug = chapterBookSlug(entry)
    default:       slug = nil
    }
